...
第一次打开前，仓库主人需要在 GitHub 打开 Pages（创建 Pages 站点是仓库管理操作，云端 token 做不了）：
...
静态文件已经在 `main` 根目录，不需要 GitHub Actions。选 Actions 反而会因为站点尚未创建而红掉。iPhone Safari 打开上面的地址即可。
...
站点从 `main` 分支根目录发布（Deploy from a branch），没有构建步骤。
